...
if (urlParams.has("loc1")) {
    ...
    if (source == 'point') {
        initial_loc1 = [-33.891, 151.1935];
        hideDivs("line-only");
        if (units === 'g') { document.getElementById("source-title").innerHTML = "Source pollutant emission rate (g/s):"; }
        else if (units === 'bubbles') {
            document.getElementById("source-title").innerHTML = "Source pollutant emission rate (bubbles/s):";
            q.value = 1000;
            h.value = 3;
            concentration_contours.value = "0.01; 0.1; 1; 10";
            document.getElementById("concentration-contours-title").innerHTML = "Concentrations, separate with ';' (bubbles/m<sup>3</sup>):"
        }
    }
    else if (source === 'line') {
        ...
        if (units === 'g') {
            ...
            concentration_contours.value = "0.01; 0.1; 1; 10";
            document.getElementById("concentration-contours-title").innerHTML = "Concentrations, separate with ';' (&micro;g/m<sup>3</sup>):"

        }
        else if (units === 'bubbles') {
            document.getElementById("source-title").innerHTML = "Source pollutant emission rate (bubbles/s/m):";
            q.value = 1000;
            h.value = 3;
            concentration_contours.value = "0.01; 0.1; 1; 10";
            document.getElementById("concentration-contours-title").innerHTML = "Concentrations, separate with ';' (bubbles/m<sup>3</sup>):"
...
function onLeftMapClick(e) {
    ...
    if (source === 'point') { markers[0].setLatLng(e.latlng); }
    else if (source === 'line') {
...
if (source === 'point') {
    ...
    map.on('ondragend', redrawContours);
    markers.push(L.marker(initial_loc1, {
        icon: customIcon // tried but didn't make something good - worth continuing with!
    }).addTo(map));//.bindPopup("I am an orange leaf.");
    map.setView([markers[0]._latlng.lat, markers[0]._latlng.lng], default_zoom) // zoom the map to the polygon
}
else if (source === 'line') {
    map.on('click', onLeftMapClick);
    map.on('ondragend', redrawContours);
    ...
    // var marker_color_0 = '#894dff';
    var marker_color = '#fff';

    markers.push(L.marker(initial_loc1, {
        icon: customIcon
    }).addTo(map));
    markers.push(L.marker(initial_loc2, {
        icon: customIcon
    }).addTo(map));

    polyline = L.polyline([markers[0]._latlng, markers[1]._latlng], {
        color: '#FFF',
        weight: 5,
        // stroke: true,
        // opacity: 1.0,
    ...
    map.setView([(markers[0]._latlng.lat + markers[1]._latlng.lat) / 2., (markers[0]._latlng.lng + markers[1]._latlng.lng) / 2.], default_zoom)
}
else if (source === 'area') {
    map.on('click', onLeftMapClick);
    map.on('contextmenu', onRightMapClick);
...
function redrawContoursFromLine() {
    ...
    nx = parseInt(resolution.value);
    ny = parseInt(nx * mapdiv.clientHeight / mapdiv.clientWidth); // use square pixels
    console.log(nx, ny)

    var bounds = map.getBounds();
    var data = new Array(nx * ny).fill(0);
    ...
    for (var m = 0; m < markers.length - 1; m++) {
        ...
        var x_1 = markers[m]._latlng.distanceTo(L.latLng(markers[m]._latlng.lat, bounds.getWest()));
        var y_1 = markers[m]._latlng.distanceTo(L.latLng(bounds.getSouth(), markers[m]._latlng.lng));
        var x_2 = markers[m + 1]._latlng.distanceTo(L.latLng(markers[m + 1]._latlng.lat, bounds.getWest()));
        var y_2 = markers[m + 1]._latlng.distanceTo(L.latLng(bounds.getSouth(), markers[m + 1]._latlng.lng));

        // check if markers are inside domain and flip sign if outside
        if (markers[m]._latlng.lng < bounds.getWest()) { x_1 = -x_1; }
        if (markers[m]._latlng.lat < bounds.getSouth()) { y_1 = -y_1; }
        if (markers[m + 1]._latlng.lng < bounds.getWest()) { x_2 = -x_2; }
        if (markers[m + 1]._latlng.lat < bounds.getSouth()) { y_2 = -y_2; }
        ...
        distance_x = map.distance(
            L.latLng(bounds.getEast(), markers[m]._latlng.lng),
            L.latLng(bounds.getWest(), markers[m]._latlng.lng)
        )
        distance_y = map.distance(
            L.latLng(markers[m]._latlng.lat, bounds.getSouth()),
            L.latLng(markers[m]._latlng.lat, bounds.getNorth())
        )
        ...
        for (var i = 0; i < nx; i++) { // for each pixel
            for (var j = 0; j < ny; j++) {
                ...
                if (isFinite(C)) { data[i + j * nx] += C * 1000; } // convert from g to µg
                // data[i+j*nx] = d_1;
            }
        }
    }
    ...
    legend_div.innerHTML += '<i style="background: ' + marker_color + '"></i><span>Road end point (left click to add, right click to reset)</span><br>';
    // legend_div.innerHTML += '<i style="background: ' + marker_color_1 + '"></i><span>End of road (right click) </span><br>';
    if (units === 'g') { legend_div.innerHTML += "<h4>Concentration (&micro;g/m<sup>3</sup>)</h4>"; }
    else if (units === 'bubbles') { legend_div.innerHTML += "<h4>Concentration (bubbles/m<sup>3</sup>)</h4>"; }
...
function redrawContoursFromPoint() {
    ...
    for (var i = 0; i < concs.length; i++) {
        ...
        if (units === 'g') { conc *= 1e-6; } // convert to micrograms!
        var col = colors[i];
        var x_p_vertices = [];
        var x_m_vertices = [];
        var y_p_vertices = [];
        ...
        while (x < max_draw) {
            ...
            if (!isFinite(y_plus) && x > 1000) { // HACK: SHOULD CHECK FOR MAX VALUE AT GROUND LEVEL INSTEAD USING KNOWN FORMULA
                ...
                lat_p = markers[0]._latlng.lat + (-x * Math.sin(theta) + y_plus * Math.cos(theta)) / 111111.;
                lon_p = markers[0]._latlng.lng + (x * Math.cos(theta) + y_plus * Math.sin(theta)) / (111111. * Math.cos(markers[0]._latlng.lat * Math.PI / 180.));

                lat_m = markers[0]._latlng.lat + (-x * Math.sin(theta) - y_plus * Math.cos(theta)) / 111111.;
                lon_m = markers[0]._latlng.lng + (x * Math.cos(theta) - y_plus * Math.sin(theta)) / (111111. * Math.cos(markers[0]._latlng.lat * Math.PI / 180.));
    ...
    for (var i = 0; i < concs.length; i++) {
        if (units === 'g') { legend_div.innerHTML += '<i style="background: ' + colors[i] + '"></i><span>' + concs[i] + ' &micro;g/m<sup>3</sup></span><br>'; }
        else if (units === 'bubbles') { legend_div.innerHTML += '<i style="background: ' + colors[i] + '"></i><span>' + concs[i] + ' bubbles/m<sup>3</sup></span><br>'; }
    }
}
...
/**
  ...
  * @param {bubbles[]} latlngA [lat, lng] point A
  * @param {bubbles[]} latlngB [lat, lng] point B
  */
