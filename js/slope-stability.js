...
window.proxy_server = '';
window.topo_server = 'https://data.scigem-eng.sydney.edu.au/elevation?verbose&';//'/' + data_source + '?';
window.topo_server_region = 'https://data.scigem-eng.sydney.edu.au/elevation?region='
...
function get_elevation_data_from_server() {
  ...
  //document.getElementById("waitingwheel").hidden = false ;
  const response = fetch ('https://data.scigem-eng.sydney.edu.au/elevationfast?ne_lat=' + bounds._northEast.lat + '&ne_lng=' + bounds._northEast.lng + '&sw_lat='+ bounds._southWest.lat + '&sw_lng=' + bounds._southWest.lng +'&nx=' + nx + '&ny=' + ny , {})
    .then( r => r.json() )
    .then( data => {
     dy = (bounds._northEast.lat-bounds._southWest.lat)/ny ;
     dx = (bounds._northEast.lng-bounds._southWest.lng)/nx ;
